...
description: "Facehash is an open-source, SVG-first React avatar component that generates deterministic faces from any string."
date: "2026-01-30"
...
Facehash is a React package and is exercised in this repository with its React
and Next.js entrypoints. It can be used in React applications built with tools
such as Next.js, Vite, and Remix.
...
It declares no bundled runtime dependencies beyond its React/React DOM peer
requirements (with Next.js as an optional peer for the Next entry). The face is
rendered as inline SVG; CSS handles sizing and optional interaction/animation.
There are no avatar image or font requests.
...
<Facehash name="user-123" variant="solid" />
<Facehash name="user-123" variant="gradient" />
...
// Reduce or disable the default dramatic 3D projection
<Facehash name="user-123" intensity3d="subtle" />
<Facehash name="user-123" intensity3d="none" />

// Hide the initial or hover interaction (both are enabled by default)
<Facehash name="anthony" showInitial={false} interactive={false} />
```
