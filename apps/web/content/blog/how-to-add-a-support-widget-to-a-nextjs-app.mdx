...
top: false
related: ["how-to-add-a-support-widget-to-a-react-app"]
published: true
---
...
Create the key in **Settings → Developers**, add the current hostname, and see
[API Keys](/docs/quickstart/api-keys) for allowlist rules.
...
That is enough to get the widget running without putting the complete widget UI
in the initial route chunk.
...
Reload the app and confirm that the trigger is styled, opens the support home
screen, and produces no `401` or `403` API response. A `401` usually means the
key is missing or invalid. A `403` usually means the current hostname is not in
the key's allowed domains. An unstyled trigger means neither (or both) CSS
entrypoints were imported.
