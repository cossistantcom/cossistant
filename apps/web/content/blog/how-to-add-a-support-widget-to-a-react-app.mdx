...
Pass the public key explicitly from the client configuration your bundler
supports:
...
A generic `COSSISTANT_API_KEY` server variable is not automatically exposed by
Create React App, Remix, or every React bundler. See
[API Keys](/docs/quickstart/api-keys) and add the current hostname to the key's
allowed domains.
...
Reload and open it now. A `401` response usually means the public key is
missing or invalid; a `403` usually means the current hostname is not allowed.
If the trigger is unstyled, confirm that you imported exactly one Cossistant CSS
entrypoint.
...
This version uses Tailwind utilities. If you chose the plain CSS setup, replace
the classes with your own stylesheet.
