...
`<IdentifySupportVisitor />` hashes its metadata and avoids sending the same
payload again. Direct `setVisitorMetadata()` calls always perform an update, so
call that method in response to a real change rather than on every render.

Updates merge the supplied keys into the contact's existing metadata. A value
of `null` is stored as `null`; it does not remove the key. Send only the fields
you intend to change.

When authentication changes on a shared browser, identify the new stable
`externalId` even if a contact is already present. A generic
`!visitor?.contact` guard can leave the previous account attached to the new
session.
