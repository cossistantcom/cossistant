...
The current dashboard manages existing conversations; it does not expose a
start-conversation action for agents.
...
The timeline provides a structured history of the conversation. Private,
deleted, filtered, and operational records may not appear to every viewer, so
do not treat it as an immutable compliance log.
...
## Build with conversations

Use the generated `Conversation` types from `@cossistant/types` for status,
priority, channel, assignment, sentiment, tags, participants, and the latest
timeline item. To build a custom widget flow, start with
[Pages & Layouts](/docs/support-component/routing) and the
[Hooks Reference](/docs/support-component/hooks). Server integrations should
use the generated OpenAPI contract in `packages/protocol/openapi.json`; private
API keys belong on the server only.
