...
- **Structured history**: See recorded joins, status changes, and resolutions
...
- **reasoning** and **tool-call**: AI reasoning/tool activity when policy allows it
- **source-url**, **source-document**, and **citation**: Knowledge provenance
- **step-start**: Boundary between agent execution steps
- **event** and **feedback**: Structured lifecycle or feedback content

Visibility and policy determine which parts a given audience receives. The
important rule is that `parts` enrich the item instead of replacing its
canonical `text`.
