...
- **Bun 1.3.1** - The version pinned by this repository ([install Bun](https://bun.sh))
...
The root `bun dev` command starts Docker Compose and every workspace `dev` task
selected by Turbo. That currently includes more than the API and dashboard, so
use the terminal output as the source of truth and stop services you do not
need. At minimum it will:

1. Start Docker Compose (Postgres + Redis containers)
2. Start the API server with Upstash Workflow in local mode
3. Start the Next.js web application, workers, and Tinybird development tasks
...
- **API server**: `http://localhost:8787` for REST, tRPC, WebSocket, and auth flows
...
Run focused checks while iterating, then run the release-relevant repository
gates before a published-package or release change:
...
Cossistant uses Bun's built-in test runner. Tests are colocated with source files using `*.test.ts` and `*.test.tsx`.
...
- **`bun dev` fails immediately**: confirm Docker Desktop is running and ports `3000`, `8787`, `5432`, `6379`, `7181`, and `8083` are not already taken.
...
Cossistant is licensed under the
[GNU Affero General Public License v3.0](https://github.com/cossistantcom/cossistant/blob/main/LICENSE).
The AGPL permits commercial use subject to its terms; it is not a
“non-commercial” license. If you need different obligations, ask
[anthony@cossistant.com](mailto:anthony@cossistant.com) about an alternative
commercial license.
