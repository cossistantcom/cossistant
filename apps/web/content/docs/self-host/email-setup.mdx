...
5. The API turns inbound replies into conversation messages and records bounce,
   complaint, and failure events for suppression.

Both provider routers currently accept but intentionally do not persist
`email.delivered` events. Do not treat this integration as delivery tracking;
use provider logs if you need delivery confirmation.
...
- delivery, bounce, complaint, and failure webhooks (delivery is accepted but not persisted)
...
Store provider keys and webhook secrets in your deployment's secret manager,
not in a committed `.env` file. Prefer workload roles over static SES keys when
your runtime supports them, rotate long-lived credentials, and keep the bridge
timestamp/replay window enforced. SES accounts must also leave sandbox mode and
have sufficient sending limits before production rollout.
...
- bounce, complaint, and failure events reach the API and update suppression state
- a delivered event is visible in provider logs (Cossistant does not persist it)
- secrets are redacted from logs and raw inbound email retention matches your privacy policy

For SES specifically, also confirm:

- `/ses/webhooks/inbound` is receiving normalized bridge payloads
- `/ses/webhooks/events` is receiving delivery, bounce, complaint, and failure events; only bounce, complaint, and failure alter Cossistant state
