...
Self-hosting starts with the application stack. Billing, storage, email, and
analytics are additional infrastructure decisions after that stack is healthy.

## Minimum deployable stack

The repository does not currently provide a one-command production deployment.
Before following the optional-service guides, plan and operate:
...
1. Provision PostgreSQL/pgvector and Redis, configure secrets and public URLs,
   run migrations, and start the API, web app, and workers.
2. Verify authentication, API health, WebSocket connectivity, background jobs,
   and backup/restore before optional integrations.
3. Read [Billing](/docs/self-host/billing) and decide whether Polar stays enabled.
4. Set up [Storage](/docs/self-host/storage) and verify uploads.
5. Configure [Email](/docs/self-host/email-setup) and test outbound plus inbound replies.
6. Configure the repository's Tinybird resources or explicitly disable
   [Analytics](/docs/self-host/analytics) and DataFast.
7. Exercise an upgrade and rollback in staging before production traffic.
