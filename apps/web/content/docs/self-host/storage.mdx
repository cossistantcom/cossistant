...
<Callout type="warning">
  The bundled module is a permissive starter, not a production-hardened storage
  baseline. It enables `force_destroy`, disables S3 public-access blocks,
  permits public reads, allows CORS from `*`, and creates long-lived IAM access
  keys. Treat every object URL as public and review every setting before using
  the module outside an isolated development environment.
</Callout>

For production, restrict CORS to your real origins, prefer private objects plus
signed/CDN delivery where possible, enable encryption/versioning/retention and
access logs, replace static keys with workload roles when your platform
supports them, rotate any remaining keys, and test backup and recovery.
...
Use encrypted remote state with locking for shared or production environments.
The state contains sensitive resource details, and the module outputs an IAM
secret. Do not commit state, plans, tfvars, or output containing credentials.

Create or select a workspace, inspect a saved plan, then apply that exact plan:

```bash
terraform workspace new dev
terraform workspace select dev
terraform plan -var-file="terraform.dev.tfvars" -out="cossistant-storage.tfplan"
terraform apply "cossistant-storage.tfplan"
```
...
- `S3_SIGNED_URL_EXPIRATION_SECONDS`: lifetime of generated upload URLs; defaults to 900 seconds
...
- production CORS, public-access, retention, encryption, credentials, and teardown behavior were reviewed explicitly

Before any `terraform destroy`, remember that the starter module's
`force_destroy` setting permits deletion of a non-empty bucket. Back up required
objects and inspect the destroy plan; a Terraform workspace is not a backup or
an isolation boundary by itself.
