...
The TSX examples use Tailwind utilities for brevity. Tailwind is not required:
replace those class names with your own CSS when you use the precompiled
`@cossistant/react/styles.css` entrypoint.
...
When you replace a slot, you inherit its accessibility responsibilities. Keep
the forwarded ref and button props, preserve the trigger's accessible name and
focus styles, make the target at least 44 by 44 CSS pixels, and announce unread
counts with meaningful text when they matter to the user.
