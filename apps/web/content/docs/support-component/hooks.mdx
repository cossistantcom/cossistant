...
description: Curated hooks for Support state, navigation, identity, conversations, and custom pages.
search:
  kind: hook
  tags:
    - react-hooks
...
- `useFeatureFlag(s)` and `useOnboarding` for typed Support state
- feedback hooks for form state and submission
...
Inside `SupportProvider`, message submission can resolve the provider client.
Typing reports require either an explicit `client` or the realtime inputs shown
in the full Support source. If you build a standalone composer and need typing
indicators, pass the client explicitly; otherwise the message still sends but
typing activity is not reported.
...
Access the events context for events emitted by custom widget code. The built-in
widget does not automatically publish lifecycle/message events in v0.2.0. The
hook returns `null` outside the widget's event provider; read the
[manual events reference](/docs/support-component/events) before using it for
analytics.
...
and the rest of the curated support type reference. This page is not an
exhaustive export list: the installed `@cossistant/react/hooks` declarations
also include conversation/timeline queries, realtime typing/seen hooks,
feature/onboarding hooks, feedback hooks, and create/send/upload mutations.
