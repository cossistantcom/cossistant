...
## Core usage in a browser

Support state also works without React. This public-key example must run in a
browser whose `Origin` is allowed for the website; do not copy it into a Next.js
server module. Server mutations use `apiKey` with a private key, as shown above.
...
- Flags can be assigned at visitor, contact, or contact-organization level; a
  flag is enabled when any applicable level contains it.
- Anonymous onboarding state belongs to the visitor. Once identified, contact
  onboarding state wins.
