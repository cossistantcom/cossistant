...
Use `theme="light"` to pin the widget to light mode instead.
...
## Starter token reference

This table covers the most common overrides, not every exported token. Use the
installed CSS entrypoint as the canonical reference for your package version;
it also defines popover, secondary, accent, input, ring, destructive
foreground, fonts, and additional background levels.
...
The current CSS uses `color-mix(in oklch, …)` for derived shades. If you target
an older browser without that syntax, override the derived `--co-theme-*`
values directly and include that browser in your visual test matrix.
