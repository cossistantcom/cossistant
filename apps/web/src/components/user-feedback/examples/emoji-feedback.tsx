					...
					<form onSubmit={feedback.handleSubmit}>
						...
						<div className="flex items-center justify-between gap-3 border-t px-4 py-3">
							...
							>
								{ratings.map((rating) => (
									<ToggleGroupItem
										aria-label={`Rate ${rating.value} out of 5`}
										className={cn(
											"size-8 p-0 text-lg opacity-45 transition-opacity hover:opacity-75",
											feedback.fields.rating.displayValue === rating.value
												? "opacity-100"
												: null
										)}
										key={rating.value}
										onMouseEnter={() =>
											feedback.handleRatingHoverChange(rating.value)
										}
